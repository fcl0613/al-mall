...
public class AccountRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        // 从数据库中获取数据
//        MallAdmin mallAdmin = mallAdminService.getOne(new LambdaQueryWrapper<MallAdmin>()
//                .eq(MallAdmin::getUsername, username));
//        if (null == mallAdmin) throw new AuthenticationException("用户不存在!");
        MallUser mallUser = userService.getOne(new LambdaQueryWrapper<MallUser>()
                .eq(MallUser::getUsername, username));
        if (null == mallUser) throw new AuthenticationException("用户不存在!");
